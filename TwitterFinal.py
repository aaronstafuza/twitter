...
API_KEY = 'WTznEN9yxV6qXhvohQJZAe5bA'
API_SECRET_KEY = '5BihtDAaIeO4DSC3qyMlK8ExUSybxXKJzJgL0UxdtZBejq53BX'
ACCESS_TOKEN = '957786457669849088-1ZyOqHtQZF2fZ2gkvkKqDt3H42JasCq'
ACCESS_TOKEN_SECRET = 'x9eMSA5U8p83HtdsdHGasPnGKviQS3Y7xs9oP9Wva5wDg'
...
# Function to select topic and approach based on recent tweet performance
def seleccionar_tema_y_enfoque():
    tema, enfoque_type = None, None
    
    try:
        response = client.get_users_tweets(id="your_twitter_user_id", tweet_fields=['public_metrics'], max_results=10)
        if response.data:
            # Find the tweet with the highest engagement (likes + retweets)
            best_tweet = max(response.data, key=lambda tweet: tweet['public_metrics']['like_count'] + tweet['public_metrics']['retweet_count'])
            best_tweet_text = best_tweet['text']
            logging.info(f"Best performing tweet: {best_tweet_text}")
            
            # Extract the tema from the best performing tweet if possible
            for t in temas:
                if t.lower() in best_tweet_text.lower():
                    tema = t
                    break
...
def schedule_tweets():
    tweet_count = 0
    while tweet_count < 24:
        publicar_tweet()
        tweet_count += 1
        time.sleep(3600)  # Wait for 1 hour before posting the next tweet

    logging.info("24 tweets posted today. Scheduling will resume tomorrow.")

if __name__ == "__main__":
    schedule.every().day.at("00:00").do(schedule_tweets)  # Start scheduling every day at midnight
    while True:
