...
import time
import logging
import requests  # For external content sourcing (e.g., news, articles)
...
# Enhanced storytelling approaches
enfoques = [
    "Tell the story of how {tema} has transformed an industry. Focus on the journey, the challenges, and the impact.",
    "Narrate a compelling success story in {tema}, detailing the breakthrough moment and what led to it.",
    "Share an inspirational story about {tema}, focusing on the individuals or teams that made a difference.",
    "Provide a case study on {tema}, highlighting the problem, the solution, and the outcomes.",
    "Describe a recent innovation in {tema}, focusing on how it's changing the landscape and future implications."
]

# Fixed hashtags for all tweets
hashtags_fijos = "#MamboDigital #MamboTrends"

# Helper function to check tweet length (280 characters total including hashtags)
...
def seleccionar_tema_y_enfoque(scores):
    ...
    enfoque = max(scores["enfoques"], key=scores["enfoques"].get) if scores["enfoques"] else random.choice(enfoques)
    return tema, enfoque
...
def generar_tweet(scores):
    tema, enfoque = seleccionar_tema_y_enfoque(scores)
    ...
    logging.info(f"Tema seleccionado: {tema}")
    ...
    try:
        ...
        tweet_text = truncate_tweet_text(tweet_text, hashtags_fijos)

        if is_tweet_within_limit(tweet_text):
            logging.info(f"Tweet within limit: {tweet_text}")
            return tweet_text, tema, enfoque
        else:
...
def publicar_tweet():
    ...
    tweet_text, tema, enfoque = generar_tweet(scores)
    ...
    if tweet_text:
        ...
        while not success and attempts < 3:  # Retry up to 3 times
            try:
                ...
                actualizar_puntajes(tema, enfoque, likes, retweets, scores)
