...
# Predefined topics and approaches (in English)
temas = [
...
def obtener_top_tweets(user_id, num_tweets=100):
    try:
        ...
        # DEBUG: Print the response to see if the API returns anything
        ...
        if response.data:
            ...
            print("No tweets found for the user.")  # Added print for testing
            return []

    except Exception as e:
        logging.error(f"Error fetching tweets: {e}", exc_info=True)
        print(f"Error fetching tweets: {e}")  # Added print for testing
        return []
